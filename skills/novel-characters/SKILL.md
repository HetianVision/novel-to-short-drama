...
左栏的半身像是面部设计基准，右栏三视图的脸照着它画。**提示词里必须要求两边一致**，否则一张图里会出现两个长相。
...
- 设定图最容易出的问题是**一张图里两个长相**（左栏一个人、右栏另一个人），拿到图先扫一眼，见 `references/sheet.md`
...
132 项断言，不调模型、不花额度，覆盖分块 / 归并 / 多语言 / 校验 / 渲染的全部确定性逻辑。改完脚本先跑这个。
