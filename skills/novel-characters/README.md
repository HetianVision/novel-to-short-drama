...
- **角色设定图** — 一张 16:9 图分三区：左侧约 34% 证件照式半身像（面部基准）、右上全身三视图、右下关键细节特写条。白底方便抠图，走 codex 内置出图（可选）
...
174 项断言，覆盖分块 / 别名归并 / 多语言 / 校验 / 渲染。不调模型、不花额度、1 秒跑完。改完脚本先跑这个。
